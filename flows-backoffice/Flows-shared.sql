...
BEGIN;
-- 1) Aggiungi colonna se manca
ALTER TABLE public.users
  ADD COLUMN IF NOT EXISTS first_login boolean;

-- 2) Imposta default a false (vale anche per i nuovi insert)
ALTER TABLE public.users
  ALTER COLUMN first_login SET DEFAULT false;

-- 3) Backfill: porta a false i NULL esistenti
UPDATE public.users
...
-- 5) Imposta true per gli utenti richiesti
UPDATE public.users
SET first_login = true
WHERE lower(username) IN ('f.pili', 'a.coppola');

-- (opzionale) verifica
-- SELECT id, username, role, first_login FROM public.users WHERE lower(username) IN ('f.pili','a.coppola');
